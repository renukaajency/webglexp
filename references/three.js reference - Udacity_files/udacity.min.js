...
{stripe_token:a})}).then(function(b){a.creditCardErrorMessage="";a.customer=b;a.customerCard=c.getDefaultCard(b);a.editingCreditCard=!1;a.enabledCreditCardButton=!0},function(b){a.creditCardErrorMessage=c.prettifyErrorResponse(b);a.enabledCreditCardButton=!0}))};d()}]);angular.module("account").factory("cancellationFlow",["modal","$q","api","$location","customers","CancellationFrame","recordActivity","dateTime","currentUser","$rootScope","$window",function(a,d,b,c,e,f,g,h,i,k,j){var l=i.fetchFullUser();return{cancelSubscription:function(b){var c=b.id,e=b.status,d=1E3*(b.trial_end||b.start),g=1E3*b.current_period_end,i=(Math.random().toString(36)+"000000000000000").slice(2,16),k=b.plan.id;"trialing"!==e&&h.calculateDaysBetweenDates(new Date(d),h.getCurrent());return l.then(function(){return a.activate("postCancelSurvey",
{subscriptionId:c,subscriptionPlanId:k,subscriptionPeriodEndDate:new Date(g),cancellationFlowToken:i}).then(function(){j.location.reload()}).catch(function(){f.setCurrentFrame("cancelSurvey")})},function(){j.location.reload()})}}}]);angular.module("account").factory("CancellationFrame",[function(){var a="cancelSurvey";return{getCurrentFrame:function(){return a},setCurrentFrame:function(d){return a=d}}}]);angular.module("account").controller("contactInfoTab",["$scope","country","currentUser","users","validator","$window",function(a,d,b,c,e,f){a.saveDisabled=!1;a.showForm=!1;a.serverSuccess="";a.serverError="";a.countries=[""].concat(d.countries);a.contactInfoErrors={};b.fetchFullUser().then(function(b){a.user=b;a.user.mailing_address||(a.user.mailing_address={address_line_1:"",address_line_2:"",city:"",region:"",postal_code:"",country:""});a.showForm=!0});a.saveContactInfo=function(){if(!a.saveDisabled){a.saveDisabled=
...
""===a.confirmedNewPassword}]});f=c.check(f);a.passwordErrors=f.errorText;f.valid?(f={key:d.getUser().key,password:{old:a.currentPassword,"new":a.newPassword}},b.put(f).then(function(){a.serverSuccess="Password successfully updated!";e()},function(b){a.serverError=b.error;a.saveDisabled=!1})):a.saveDisabled=!1}}}]);angular.module("account").controller("subscriptionCancellation",["$scope","modal","$q","api","$location","customers","$window","products","recordActivity","dateTime","currentUser","$rootScope","cancellationFlow","CancellationFrame",function(a,d,b,c,e,f,g,h,i,k,j,l,m,n){a.cancelSubscription=m.cancelSubscription;a.confirmSubscriptionCancellation=function(b){a.state="checking";return f.removeSubscription("me",b).then(function(){a.sendCancellationActivity("cancel")}).then(function(){a.setCurrentFrame("cancelled")})};
a.getCurrentFrame=n.getCurrentFrame;a.setCurrentFrame=n.setCurrentFrame;a.survey={};a.surveyChoices=[{name:"I'm not able to dedicate enough time to this program",code:"notEnoughTime",description:'We know it\'s tough finding time to learn in your busy schedule. Have you tried <a href="https://discussions.udacity.com/categories" target="_blank">creating a study group</a> with your fellow students? Remember all you need to make progress is 30 minutes of learning time.'},{name:"I wasn't too happy with the quality of your content",
...
{image:"//lh3.googleusercontent.com/a2NwzSoXvaOiPSv6m1Hdq9M305O7ur2n2x--EvtygezETgVViaa0ls8fL9Kfw0wFf0npJOuCWH1J-0dC6dA=s0#w=120&h=120",story:'<strong>Christian</strong>, a former lawyer, co-founded a startup with the help of skills he learned from the Data Science Nanodegree. <br><a href="http://blog.udacity.com/2015/03/udacity-student-spotlight-christian-a-complete-career-shift.html" target="_blank"> View Christian\'s Journey </a>'}];a.studentStory=b[Math.floor(Math.random()*(b.length-1))];a.completionStats=
{time:"...",projects:"...",quizzes:"..."};a.getCourseProgress=function(){return h.get(a.getData().subscriptionPlanId).then(function(b){a.subscriptionCourseKey=b._course_groups[0].course_ref.key;return c.get("/users/node_progress/"+a.subscriptionCourseKey).then(function(b){a.completionStats.time=Math.round(b.data.stats.time/60);a.completionStats.projects=b.data.stats.projects;a.completionStats.quizzes=b.data.stats.quizzes;return b.data.stats})})};a.sufficientTimeInClass=function(){return 30<=a.completionStats.time};
a.closeModal=function(){d.cancel()};a.keepLearning=function(){a.closeModal();g.location.href="/me"};a.confirmModal=function(){d.confirm()};a.startConversation=function(){return c.post("/email/send_cancellation_reason",{email_problem:a.survey.reason,email_comment:a.survey.comment,node_id:a.subscriptionCourseKey}).then(function(a){return a.data})};var p=function(){switch(a.getCurrentFrame()){case "cancelSurvey":return 1;case "accomplishments":return 2;case "hereToHelp":return 3;case "startAConvo":return 4;
case "rogerThat":return 5;case "reminder":return 6;case "cancelled":return 7}};a.sendCancellationActivity=function(b,c,e){var d=a.getData().cancellationFlowToken,f=p(),g=a.subscriptionCourseKey,h;h="nd"===a.subscriptionCourseKey.slice(0,2)?"Degree":"Course";var k,j;c&&(k=c);e&&(j=e);return i.cancellationFlowStep(d,f,g,h,b,k,j)}}]);angular.module("utility").service("showChat",["currentUser","$q",function(a,d){var b=null;return{getGroupChatPromise:function(){b||(b=d.defer(),a.fetchFullUser().then(function(c){a.isStaff()?b.resolve(!0):a.fetchEnrolledAndGraduatedNanodegreeKeys().then(function(a){if(1===a.length&&"nd801"===a[0])b.resolve(!1);else{var d=_.filter(c._enrollments,function(b){return _.contains(a,b.node_key)});_.find(d,function(a){if(a.first_paid_enrollment_time)return new Date(a.first_paid_enrollment_time)>new Date(2015,
...
a.getSelectedTechnologyNames(),c="",d=a.activeCategory&&a.activeCategory.name;1===b.length?c=_.first(b)+" ":1<b.length&&(c=_.initial(b).join(", ")+", and "+_.last(b)+" ");return"Browse "+d+" Course and Nanodegree Programs to help advance your career. You'll learn "+c+"from industry experts from Google, Facebook, Twitter and Cloudera."};h.list().then(function(b){a.courses=_.map(_.union(b.degrees,b.courses),function(a){a.affiliates.length&&(a.affiliate_program_refs=_.map(a.affiliates,function(a){return{key:a.key}}));
return a});a.affiliateMap=_.reduce(a.courses,function(a,b){b.affiliates.length&&_.each(b.affiliates,function(b){a[b.key]=b});return a},{});a.tracks=b.tracks;return{courses:a.courses,affiliates:a.affiliateMap,tracks:a.tracks}}).then(function(b){a.filters=g.initializeFilters(b.courses,b.affiliates,d.search());d.search("");var e=function(){var e=_.find(b.tracks,function(a){return c("slugify")(a.name)===d.path().substr(1)});e?a.activeCategory=e:d.path("all")};a.$on("$locationChangeSuccess",e);e();a.loadingComplete=
!0})}]);angular.module("courseCatalog").constant("AFFILIATE_PRIORITIES",{facebook:1E3,google:1E3,Salesforce:1E3,gt:8E3,sjsu:8E3,twentythree:9E3}).factory("courseCatalogHelper",["AFFILIATE_PRIORITIES",function(a){var d="ma004 ma006 ma008 bio110 ph100 ps001".split(" "),b=[{id:"skills",name:"Skill Level",makeTitleCase:!0,generateOptions:function(a,b,d){return _.map(_.sortBy(_.unique(_.map(a,function(a){return a.level})).concat("new to tech"),function(a){return{beginner:1,intermediate:2,advanced:3}[a]||0}),function(a){return{name:a,
...
"Group"===a.group_ref.ref&&e(a.group_ref.key)});a.ready=!0})}]);angular.module("overview").directive("nanodegreeEnrollmentActions",["cacheVersion","currentUser","groups","$location","dateTime","$window","cookieJar","googleTagManager","universalAnalytics",function(a,d,b,c,e,f,g,h,i){return{restrict:"A",scope:{enrollmentActions:"="},templateUrl:"/templates/overview/nanodegree_enrollment_actions.html?"+a,controller:"enrollmentActionsHelper",link:function(a){var b=function(a){return{nd000:{start:"August 11 at 12:00 pm Eastern Time",end:"August 20 at 11:59 pm Eastern Time"},
nd001:{start:"August 11 at 12:00 pm Eastern Time",end:"August 20 at 11:59 pm Eastern Time"},nd002:{start:"August 11 at 12:00 pm Eastern Time",end:"August 20 at 11:59 pm Eastern Time"},nd003:{start:"August 11 at 12:00 pm Eastern Time",end:"August 20 at 11:59 pm Eastern Time"},nd004:{start:"August 11 at 12:00 pm Eastern Time",end:"August 20 at 11:59 pm Eastern Time"},nd801:{start:"August 11 at 12:00 pm Eastern Time",end:"August 20 at 11:59 pm Eastern Time"}}[a]||{}},c=function(){h.pushTrackingEvent("nanodegreeAccess");
...
first_name:a.firstName,last_name:a.lastName,password:a.password,tos_accepted:!0}).then(t,s):(a.fieldErrors=b.errorText,a.buttonDisabled=!1)};a.signUpFacebook=function(){return j.getAuthResponse().then(function(a){return j.getMeResponse().then(function(b){return f.registerUser({email_address:b.email,first_name:b.first_name,last_name:b.last_name,facebook:{signed_request:a.signedRequest},tos_accepted:!0}).then(t,s)},w)},v)};a.signUpGoogle=function(){k.getToken(!1).then(function(a){return e.put({google:{token:a.code}}).then(t,
function(){return p(k.getMeResponse,5,100).then(function(a){return k.getToken(!0).then(function(b){return f.registerUser({email_address:a.email,first_name:a.given_name,last_name:a.family_name,google:{token:b.code},tos_accepted:!0}).then(t,s)},v)},w)})},v)};q.exists("emailAddressForSignUp")&&(a.email=q.get("emailAddressForSignUp"))}]);angular.module("account").controller("viewerSettings",["$scope","currentUser","users","techDetect","$window",function(a,d,b,c){a.isBrowserIE="Explorer"===c.browserStats().browser;a.videoQualities=[{label:"Automatic",value:"default"},{label:"360p",value:"medium"},{label:"480p",value:"large"},{label:"720p",value:"hd720"},{label:"1080p",value:"hd1080"}];a.playbackRates=[{label:"0.25x",value:0.25},{label:"0.5x",value:0.5},{label:"Normal",value:1},{label:"1.5x",value:1.5},{label:"2x",value:2}];var e=d.getUser();
...
(e=c.search().products.split(","),d.begin(e),a.flow=d.getCurrentFlowData()):window.history.back()}});a.getCurrentDate=function(){return i.getCurrent()};a.isProductLaunched=function(a){return a&&a.launch_date&&new Date(a.launch_date)<i.getCurrent()};a.isProductNotLaunched=function(a){return a&&!a.was_launched};a.getGoToNanodegreeLink=function(){return a.flow&&"nd001"===a.flow.uriEncodedCourseKey?"/teams/#/onboarding/"+a.flow.uriEncodedCourseKey:"/me"};a.getDatePlusDays=function(a,b){return a?new Date(a.getTime()+
864E5*b):null};a.step={};a.submitStep=function(b){a.step.submitting=!0;d.submitStep(b).then(function(){a.step.submitting=!1;$("html,body").animate({scrollTop:0})})};a.getCacheBustingUrl=function(a){return a&&a+"?"+g};a.currentUserName=f.getFullName();a.currentUserEmail=f.get("email").address;a.currentUserNickname=f.get("nickname");a.getCurrentUserIsVerified=function(){return f.get("email")._verified};a.getCurrentStep=d.getCurrentStep;a.getCurrentFlowSteps=d.getCurrentFlowSteps;k(function(){a.getCurrentUserIsVerified()||
h.activate("verify email")},1)}]);angular.module("page.payment",["core"]).config(["$routeProvider",function(a){a.when("/:stepNumber",{}).otherwise({redirectTo:"/0"})}]);angular.module("payment").constant("DAY_IN_MILLISECONDS",864E5).factory("fullClassExperienceFlowDefinition",["$q","stripeCreditCard","validator","api","currentUser","products","customers","couponCode","users","$timeout","$log","nodes","DAY_IN_MILLISECONDS","userTrials","googleTagManager","universalAnalytics",function(a,d,b,c,e,f,g,h,i,k,j,l,m,n,p,q){var r=function(a){return"card_declined"===a.code?"Your card was declined. Please try again. If this issue persists, please contact payment-support@udacity.com.":
"processing_error"===a.code?"An error occurred while processing your card. Please try again.If this issue persists, please contact payment-support@udacity.com.":a.code?a.message:"An error occurred while processing your card. Please check your information and try again. If this issue persists, please contact payment-support@udacity.com."},t=function(a){return a&&_.map(a._course_groups,function(a){return a.course_ref&&a.course_ref.key})},v=function(b,d){var e={product_keys:d.productKeys};b&&b.id&&(e.stripe_token=
...
{valid:!1};return null});i.push(k)}a.all(i).then(function(a){e.product=a[0];var b=_.first(e.product._course_groups).course_ref.key;l.get(b).then(function(a){var b=a.catalog_entry._image?a.catalog_entry._image.serving_url:null;e.confirmationStepLabel={Degree:"Start Nanodegree",Course:"Start course"}[a._semantic_type];e.course={key:a.key,imageUrl:b,isNanodegree:"Degree"===a._semantic_type};p.pushTrackingEvent("checkoutStart",{productName:e.product.name,productPrice:e.product.amount,orderNodeKey:e.course.key,
isNanodegree:"Degree"===a._semantic_type})});b=e.product.trial_period_days;e.inTrialPeriod=0<b;e.trialPeriodStarts=new Date(Date.now()+b*m);e.customer=a[1];e.customerCard=d.getDefaultCard(e.customer);e.cardIsLoaded=!0;e.uriEncodedCourseKey=window.encodeURIComponent(t(e.product))})},onJump:function(a){return a.paymentConfirmed?!1:!a.paymentConfirmed},onSubmit:function(b,c){if(c.customerCard)return v(!1,c);var e=b.creditCard||{},f=d.checkValidity(e);c.creditCardErrors=f.errorText;if(!f.valid)return!1;
f=(c.product&&c.product.name)+", "+t(c.product);return d.createPaymentToken(f,e).then(function(a){return v(a,c)},function(b){c.generalPaymentFormError=r(b.error);return a.reject(b)})}},{getLabel:function(a){return a.confirmationStepLabel},view:"/templates/angular/payment/payment_confirmation.html",onStart:function(a,b){b.paymentConfirmed=!0;b.payment=a;var c=b.product&&(b.product.amount||0)/100,d=null;try{d=_.find(a.customer.subscriptions,function(a){return a.plan.id===b.product.key}).id}catch(e){j.error("Failed to get Transaction Id",
e)}q.trackTransaction({transactionId:d,transactionTotal:c,transactionAffiliation:"Udacity",transactionProducts:[{name:b.product&&b.product.name||"Missing product name, FILL IN IN CONTENT EDITOR ASAP FOR "+_.first(t(b.product)),sku:"DD44",price:c,quantity:1,category:_.first(t(b.product))}]});p.pushTrackingEvent("CompletedOrder",{transactionVal:c,transactionOrderId:d,transactionPromoCode:h,orderNodeKey:b.course.key,isNanodegree:b.course.isNanodegree})}}]}]);angular.module("payment").factory("paymentFlow",["$q","paymentState","$location",function(a,d,b){return{begin:function(a){a=d.startFlow(a);if(a.onStart)a.onStart(null,d.getCurrentFlowData());b.path("/"+d.getCurrentStep().number.toString())},submitStep:function(c){var e=a.defer();(c=d.getCurrentStep().onSubmit(c,d.getCurrentFlowData()))?a.when(c).then(function(a){var c=d.goToNextStep();if(c.onStart)c.onStart(a,d.getCurrentFlowData());e.resolve();b.path("/"+c.number.toString())},function(){e.resolve()}):
...
fetchStudyGroups:function(){return c.getStudyGroups("me")},getEmail:function(){return h.getEmail(j)},setUser:function(a){angular.copy(angular.copy(a),j)},fetchFullUser:function(a){var b=a?{cacheBust:Math.random()}:{};if(a)return c.get("me",b).then(function(a){n.setUser(a);return a});l||(l=c.get("me",b).then(function(a){n.setUser(a);return a}));return l},fetchCertificates:function(){return j._certificates?d.when(j._certificates):c.getCertificates("me").then(function(a){return j._certificates=a})},
checkIfFirstChatVisit:function(){return c.checkIfFirstChatVisit("me")},setFirstChatVisit:function(){return c.setFirstChatVisit("me")},checkNewHomeBanner:function(){return c.checkNewHomeBanner("me")},setNewHomeBanner:function(){return c.setNewHomeBanner("me")},fetchCohorts:function(){m||(m=n.fetchFullUser().then(function(a){return d.all(_.map(a._cohort_keys,function(a){return i.get(a)}))},function(b){a.error(b)}));return m},userIsLoggedIn:function(){return j._registered},isEnrolledIn:function(a){return _.some(j._enrollments,
function(b){return b.node_key===a&&"enrolled"===b.state})},isEnrolledForCreditIn:function(a){var b=["151101088","150207572","151148782","151059885","151254701"];return void 0!==_.find(j._enrollments,function(c){var d=_.pluck(c.group_refs,"key");return c.node_key===a&&"enrolled"===c.state&&0<_.intersection(d,b).length})},isCourseEmailSubscribed:function(a){return 0<_.filter(j._enrollments,function(b){return b.node_key===a&&b.email_ok}).length},enroll:function(a,d,e){d=d||"enrolled";return c.setEnrollments("me",
[a],d).then(function(a){j._enrollments=j._enrollments||[];_.each(a,function(a){var b=_.find(j._enrollments,function(b){return b.node_key===a.node_key});b?j._enrollments[_.indexOf(j._enrollments,b)]=a:j._enrollments.push(a)});if(1===a.length){b.$broadcast("modifiedEnrollment",a);var c=d.slice(0,1).toUpperCase()+d.slice(1,-2),c=k.trackEvent({category:"Enrollment",action:c,label:a[0].node_key});return e?c.then(function(){return a},function(){console.warn("Failed to push enrollment to Universal Analytics");
...
k};a.userIsLoggedIn=s.userIsLoggedIn();a.canEditNotes=D;a.course=null;a.lessons=null;a.lesson=null;a.morsels=null;a.morsel=null;a.tidbit=null;var u=b.path().match("/"+m.course+"([^/]+)"),u=u?u[1]:null;a.courseKeyFromUrl=u;u||(window.location.href="/");t.addEventListener("sessionExpired",function(){window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path())});v&&(window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path()));
...
b=b.indexInLesson;b<c-1?a.goToMorsel(a.morsels[b+1]):(h.pauseVideo(),a.course?s.userIsLoggedIn()&&s.isEnrolledIn(a.course.key)?a.lessons&&a.lesson.key===a.lessons[a.lessons.length-1].key?(a.enrollment=_.find(s.get("_enrollments"),function(b){return b.node_key===a.course.key}),w.activate("endOfCourse",{courseIsComplete:a.course.catalog_entry.complete,courseKey:a.course.key,isPaidEnrolled:x.isCurrentEnrolled(a.enrollment,a.course),isGraduated:x.isGraduated(a.enrollment,a.course)})):w.activate("endOfLesson",
{lessonKey:a.lesson.key,userIsLoggedIn:s.userIsLoggedIn()}):s.userIsLoggedIn()?w.activate("endOfLessonEnrollGate",{lessonKey:a.lesson.key,userIsLoggedIn:s.userIsLoggedIn()}):w.activate("endOfLessonSignInGate",{lessonKey:a.lesson.key,userIsLoggedIn:s.userIsLoggedIn()}):w.activate("endOfIsolatedContent"))};a.goToPreviousMorsel=function(){var b=a.morsel;b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;0<b?a.goToMorsel(a.morsels[b-1]):console.error("Can't go to previous; at beginning of lesson")};
...
{projection:"image"}).then(function(b){b&&b._image_url&&a.partners[d].push({imageUrl:b._image_url,name:b.name})})})};a.fetchCards=function(b,c){return d.batchGet(b).then(function(d){return _.without(_.map(b,function(b){var e=!1,k=d[b];if(d[b]){var j=null,l=k.catalog_entry;l&&l._image&&(j=l._image.serving_url);c&&(k.affiliate_program_refs&&0<k.affiliate_program_refs.length)&&"udacity"!==k.affiliate_program_refs[0].key&&(e=!0,a._fetchAffiliate(k.affiliate_program_refs,b));return{key:b,_slug:k._slug,
servingUrl:j,title:k.title,isNanodegree:"Degree"===k._semantic_type,showPartner:e&&c}}return null}),null)})};a._setLengthOfRow=function(a,b){return!b||6<b?6>=a?a:0===a%6?6:0===a%5?5:0===a%4?4:0===a%3?3:6:b};a.setUpCards=function(b,d,g,h,i){b?(a.lengthOfRow=a._setLengthOfRow(b.length,d),a.columnWidthClass="col-xs-"+a._calculateColumnWidth(a.lengthOfRow),a.cardImageSize=[1200/a.lengthOfRow,0],angular.isArray(h)&&2===h.length?a.cardImageSize=h:angular.isNumber(h)&&(a.cardImageSize=[h,0]),i?1===i?a.columnWidthClass=
